...
type alias BingoEntry = String

type alias BingoEntries  = List BingoEntry
...
allEntries = ["Krishna Starbucks reference"
            , "\"You're on mute\""
...
update msg model =
    case msg of
        ...
        ToggleTile selected ->
            let newSelected = if List.member selected model.selected then (remove selected model.selected) else (selected :: model.selected)
            in ( { model | selected = newSelected},  Cmd.none)
        _ ->
            (model, Cmd.none)
...
bingoTileView selected tile = div [ class "bingo-tile",  class (if List.member tile selected then "selected" else "unselected"), onClick <| (ToggleTile tile)] [ text tile ]

view : Model -> Html Msg
view model =
    div [] [
         h1 [] [ text "Bingo"]
        , div [ class "bingo-card"] (List.map (bingoTileView model.selected) <| model.tiles)
    ]
